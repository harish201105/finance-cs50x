...
def index():
    ...
    transactions_db = db.execute("SELECT symbol, shares, price, total FROM transactions WHERE user_id = ? GROUP BY symbol", user_id)
...
def buy():
    ...
    if request.method == "GET":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, total, date) VALUES (?, ?, ?, ?, ?, ?)", user_id, stock["symbol"], shares, stock["price"], round(shares*stock["price"], 2), date)
...
def quote():
    ...
    if request.method == "GET":
        ...
        return render_template("quoted.html", name = stock["name"], price = str(stock["price"]), symbol = stock["symbol"])
...
def sell():
    ...
    if request.method == "GET":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, total, date) VALUES (?, ?, ?, ?, ?, ?)", user_id, stock["symbol"], (-1)*shares, stock["price"], shares*stock["price"], date)
